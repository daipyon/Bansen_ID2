...
async function Start() {
  if (Error_Text.length == 0) {
    ...
    // CPN(AE列)を取得
    const Cpn = await RPA.Google.Spreadsheet.getValues({
      spreadsheetId: `${SSID}`,
      range: `${SSName}!AE${StartRow}:AE${LastRow}`
    });
    // 作業用フラグ(AG列)を取得
    const WorkRow = await RPA.Google.Spreadsheet.getValues({
      spreadsheetId: `${SSID}`,
      range: `${SSName}!AG${StartRow}:AG${LastRow}`
    });
    for (let i in WorkRow) {
      ...
      if (WorkRow[i][0] == errortext) {
        ...
        WorkData = await RPA.Google.Spreadsheet.getValues({
          spreadsheetId: `${SSID}`,
          range: `${SSName}!A${Row}:AD${Row}`
        });
        await RPA.Logger.info(WorkData);
        // AG列に"作業中"と記載
        await RPA.Google.Spreadsheet.setValues({
          spreadsheetId: `${SSID}`,
          range: `${SSName}!AG${Row}:AG${Row}`,
          values: [['作業中']]
        });
        await RPA.Logger.info(
...
const ErrorText1 = [[errortext, '同じキャンペーン名が既に存在しています']];
const ErrorText2 = [[errortext, 'キャンペーン名が65文字以上です']];
const ErrorText3 = [[errortext, '必須入力です']];
...
async function PatternInput(pattern1, pattern2, pattern3, pattern4) {
  ...
  if (pattern4) {
    ...
    await RPA.WebBrowser.sendKeys(CampaignSpecifiedSeriesId, [WorkData[0][19]]);
  }
}
...
async function Cluster() {
  ...
  if (WorkData[0][12] == '全て選択') {
    await RPA.Logger.info(
      ' "全て選択" にチェックを入れ、"外部メディア" のチェックを外します'
    );
    // "外部メディア"のチェックを外す
    const Uncheck: WebElement = await RPA.WebBrowser.driver.executeScript(
      `return document.getElementsByName('campaign.clusterIds')[0].children[1].children[0].children[1].children[6]`
    );
    ...
    // 外部メディアは「クラスタ」の列には記載しない　→　後日共有
    if (ValuesM.includes('外部メディア')) {
      await RPA.Logger.info(' "外部メディア" にチェックを入れます');
      // 外部メディア（番宣のみ）を取得
      const Media: WebElement = await RPA.WebBrowser.driver.executeScript(
        `return document.getElementsByName('campaign.clusterIds')[0].children[1].children[0].children[1].children[6]`
      );
...
async function CampaignIdSerach() {
  while (LoadingFlag == 'false') {
    const JudgeCampaignId = await RPA.Google.Spreadsheet.getValues({
      spreadsheetId: `${SSID}`,
      range: `${SSName2}!BN${Row - 1}:BN${Row - 1}`
    });
    // キャンペーンIDで検索
    await RPA.WebBrowser.wait(
      RPA.WebBrowser.Until.elementLocated({
    ...
    await RPA.sleep(100);
    await RPA.WebBrowser.sendKeys(SearchByCampaignId, JudgeCampaignId[0][0]);
    await RPA.sleep(5000);
    try {
...
async function AdvertisementStart() {
  ...
  if (WorkData[0][20].length < 1) {
    ...
    await RPA.sleep(100);
    await RPA.WebBrowser.sendKeys(DateRangeStart2, [WorkData[0][25]]);
    await RPA.sleep(300);
    // 配信期間：終了(AA列)を入力
    ...
    await RPA.sleep(100);
    await RPA.WebBrowser.sendKeys(DateRangeEnd2, [WorkData[0][26]]);
    await RPA.sleep(300);
    // 配信期間のOKボタンを押下
    ...
    await RPA.Logger.info('シートの開始日 → ' + WorkData[0][25]);
    ...
    await RPA.Logger.info('シートの終了日 → ' + WorkData[0][26]);
    if (
    ...
    if (WorkData[0][27].length < 1) {
      ...
      await RPA.WebBrowser.sendKeys(ImpWeight, [WorkData[0][27]]);
    }
    // 予約放送枠ID(D列)を入力
...
async function AdJudgeError() {
  ...
  if (
    WorkData[0][25].length < 1 ||
    WorkData[0][26].length < 1 ||
    (WorkData[0][25].length < 1 && WorkData[0][26].length < 1)
  ) {
...
async function GetAdvertisementId() {
  ...
  await RPA.Google.Spreadsheet.setValues({
    spreadsheetId: `${SSID}`,
    range: `${SSName2}!BO${Row - 1}:BO${Row - 1}`,
