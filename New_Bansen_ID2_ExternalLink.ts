...
async function Start() {
  if (Error_Text.length == 0) {
    ...
    SheetData = await RPA.Google.Spreadsheet.getValues({
      spreadsheetId: `${SSID}`,
      range: `${SSName3}!A${StartRow}:AG${LastRow}`
    });
    // 作業用フラグ(AK列)が"完了"の行まで回す
    for (let i in SheetData) {
      ...
      if (SheetData[i][32] == errortext) {
        ...
        WorkData = await RPA.Google.Spreadsheet.getValues({
          spreadsheetId: `${SSID}`,
          range: `${SSName3}!A${Row}:AG${Row}`
        });
        await RPA.Logger.info(WorkData);
        // 作業用フラグ(AK列)に"作業中"と記載
        await RPA.Google.Spreadsheet.setValues({
          spreadsheetId: `${SSID}`,
          range: `${SSName3}!AK${Row}:AK${Row}`,
          values: [['作業中']]
        });
        await RPA.Logger.info(
...
async function Work() {
  try {
    ...
    if (WorkData[0][0] != '外部リンク付き番宣' && WorkData[0][0] != '買える') {
      ...
      await ErrorSetValue(`AK${Row}`);
      await RPA.Google.Spreadsheet.setValues({
        spreadsheetId: `${SSID}`,
        range: `${SSName3}!BD${Row}:BD${Row}`,
...
async function AdvertisementStart() {
  ...
  if (WorkData[0][22].length < 1) {
    ...
    await RPA.Google.Spreadsheet.setValues({
      spreadsheetId: `${SSID}`,
      range: `${SSName3}!BE${Row}:BE${Row}`,
      values: [['シートにIDの記載がありません']]
    });
    await Start();
  } else {
    await RPA.WebBrowser.sendKeys(CreativeId, [WorkData[0][22]]);
    await RPA.sleep(3000);
    ...
    if (WorkData[0][31] == '広告なしで ビデオをみる') {
      const WithoutAd = await WorkData[0][31].slice(0, -6);
      await RPA.WebBrowser.sendKeys(Text, [WithoutAd]);
      await Text.sendKeys(Key.ENTER);
    } else {
      await RPA.WebBrowser.sendKeys(Text, [WorkData[0][31]]);
      await Text.sendKeys(Key.ENTER);
    }
    // URL(AG列)を入力
    ...
    if (WorkData[0][32].length < 1) {
      await RPA.WebBrowser.mouseClick(Url);
    } else {
      await RPA.WebBrowser.sendKeys(Url, [WorkData[0][32]]);
      await RPA.sleep(3000);
...
async function AdJudgeError() {
  ...
  if (WorkData[0][30].length < 1) {
    ...
    await RPA.Google.Spreadsheet.setValues({
      spreadsheetId: `${SSID}`,
      range: `${SSName3}!AW${Row}:AW${Row}`,
      values: [[`【広告作成】${ErrorText4}`]]
    });
  }
  if (WorkData[0][31].length < 1) {
    RPA.Logger.info(`エラー【テキストを選択してください】`);
    await ErrorSetValue(`AK${Row}`);
    await RPA.Google.Spreadsheet.setValues({
      spreadsheetId: `${SSID}`,
      range: `${SSName3}!BB${Row}:BB${Row}`,
...
async function GetAdvertisementId() {
  ...
  // キャンペーン・広告貼り付けシートのBJ列に発番した広告IDを記載
  await RPA.Google.Spreadsheet.setValues({
    spreadsheetId: `${SSID}`,
    range: `${SSName4}!BJ${Row - 1}:BJ${Row - 1}`,
